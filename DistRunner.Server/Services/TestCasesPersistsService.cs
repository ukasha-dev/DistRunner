// <copyright file="TestCasesPersistsService.cs" company="Your Company">
...
namespace DISTRUNNER.Server.Services;
...
public class TestCasesPersistsService
{
    ...
    public TestCasesPersistsService(
        DISTRUNNERRepository DISTRUNNERRepository,
        IJsonSerializer jsonSerializer,
        IFileProvider fileProvider,
        IPathProvider pathProvider,
        IDirectoryProvider directoryProvider)
    {
        _DISTRUNNERRepository = DISTRUNNERRepository;
    ...
    public async Task PersistsHistoryToFileAsync()
    {
        try
        {
            ...
            var testCasesHistoryEntries = await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<TestCaseHistory>().ConfigureAwait(false);
            var historyEntries = await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<TestCaseHistoryEntry>().ConfigureAwait(false);
        ...
        try
        {
            ...
            if (testCaseHistoryDtoCollection.Any())
            {
                foreach (var testCaseHistoryDto in testCaseHistoryDtoCollection)
                {
                    ...
                    var createdTestCaseHistory = await _DISTRUNNERRepository.InsertWithSaveAsync(testCaseHistory).ConfigureAwait(false);
                    ...
                    if (testCaseHistoryDto.Durations.Any())
                    {
                        foreach (var currentDuration in testCaseHistoryDto.Durations)
                        {
                            ...
                            await _DISTRUNNERRepository.InsertWithSaveAsync(testCaseHistoryEntry).ConfigureAwait(false);
                        }
                    }
                }
            }
    ...
    {
        var testCasesHistoryFileNamePath = _pathProvider.Combine(GetAppDataDISTRUNNERFolder(), TestCasesHistoryFileName);
        return testCasesHistoryFileNamePath;
    }

    private string GetAppDataDISTRUNNERFolder()
    {
        var appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        var DISTRUNNERAppDataFolder = _pathProvider.Combine(appData, "DISTRUNNER");
        _directoryProvider.CreateDirectory(DISTRUNNERAppDataFolder);
        return DISTRUNNERAppDataFolder;
    }
}
