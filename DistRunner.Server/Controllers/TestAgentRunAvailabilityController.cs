// <copyright file="TestAgentRunAvailabilityController.cs" company="Your Company">
...
public class TestAgentRunAvailabilityController : Controller
{
    ...
    public TestAgentRunAvailabilityController(ILogger<TestAgentRunAvailabilityController> logger, DISTRUNNERRepository repository)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> GetLastTestAgentRunAvailabilityForTestRun([FromBody] int id)
    {
        try
        {
            var testRunAvailability = (await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<TestAgentRunAvailability>().ConfigureAwait(false)).LastOrDefault(x => x.TestAgentRunId.Equals(id));
    ...
    public async Task<IActionResult> GetTestAgentRunAvailability([FromBody] int id)
    {
        try
        {
            var testRunAvailability = await _DISTRUNNERRepository.GetByIdAsync<TestAgentRunAvailability>(id).ConfigureAwait(false);
            if (testRunAvailability == null)
            {
    ...
    public async Task<IActionResult> GetTestRunAvailabilities()
    {
        try
        {
            var testAvailabilities = await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<TestAgentRunAvailability>().ConfigureAwait(false);
    ...
    public async Task<IActionResult> CreateTestAgentRunAvailabilityAsync([FromBody] TestAgentRunAvailabilityDto testRunAvailabilityDto)
    {
        ...
        var result = await _DISTRUNNERRepository.InsertWithSaveAsync(testRunAvailability).ConfigureAwait(false);
    ...
    public async Task<IActionResult> UpdateTestAgentRunAvailabilityAsync([FromBody] KeyValuePair<int, TestAgentRunAvailabilityDto> updateObject)
    {
        ...
        var entityToBeUpdated = await _DISTRUNNERRepository.GetByIdAsync<TestAgentRunAvailability>(updateObject.Key).ConfigureAwait(false);
    ...
    {
        var entityToBeRemoved = await _DISTRUNNERRepository.GetByIdAsync<TestAgentRunAvailability>(id).ConfigureAwait(false);
