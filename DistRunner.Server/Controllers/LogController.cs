// <copyright file="LogController.cs" company="Your Company">
...
public class LogController : Controller
{
    ...
    public LogController(ILogger<LogController> logger, DISTRUNNERRepository repository)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> GetLog([FromBody] int id)
    {
        try
        {
            var log = await _DISTRUNNERRepository.GetByIdAsync<Log>(id).ConfigureAwait(false);
            if (log == null)
            {
    ...
    public async Task<IActionResult> GetLogs()
    {
        try
        {
            var logs = await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<Log>().ConfigureAwait(false);
    ...
    public async Task<IActionResult> UpdateLogAsync([FromBody] KeyValuePair<int, LogDto> updateObject)
    {
        ...
        var entityToBeUpdated = await _DISTRUNNERRepository.GetByIdAsync<Log>(updateObject.Key).ConfigureAwait(false);
    ...
    {
        var entityToBeRemoved = await _DISTRUNNERRepository.GetByIdAsync<Log>(id).ConfigureAwait(false);
