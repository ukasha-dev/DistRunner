// <copyright file="TestRunAvailabilityController.cs" company="Your Company">
...
public class TestRunAvailabilityController : Controller
{
    ...
    public TestRunAvailabilityController(ILogger<TestRunAvailabilityController> logger, DISTRUNNERRepository repository)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> GetLastTestRunAvailabilityForTestRun([FromBody] Guid id)
    {
        try
        {
            var testRunAvailability = (await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<TestRunAvailability>().ConfigureAwait(false)).LastOrDefault(x => x.TestRunId.Equals(id));
    ...
    public async Task<IActionResult> GetTestRunAvailability([FromBody] int id)
    {
        try
        {
            var testRunAvailability = await _DISTRUNNERRepository.GetByIdAsync<TestRunAvailability>(id).ConfigureAwait(false);
            if (testRunAvailability == null)
            {
    ...
    public async Task<IActionResult> GetTestRunAvailabilities()
    {
        try
        {
            var testAvailabilities = await _DISTRUNNERRepository.GetAllQueryWithRefreshAsync<TestRunAvailability>().ConfigureAwait(false);
    ...
    public async Task<IActionResult> UpdateTestRunAvailabilityAsync([FromBody] KeyValuePair<int, TestRunAvailabilityDto> updateObject)
    {
        ...
        var entityToBeUpdated = await _DISTRUNNERRepository.GetByIdAsync<TestRunAvailability>(updateObject.Key).ConfigureAwait(false);
    ...
    {
        var entityToBeRemoved = await _DISTRUNNERRepository.GetByIdAsync<TestRunAvailability>(id).ConfigureAwait(false);
